...
class VaxSeries {
  ...
  bool canSkip(
      ...
      seriesDose.conditionalSkip?.any((conditionalSkip) =>
  ...
  bool evaluateSkipCondition(ConditionalSkip conditionalSkip,
    ...
    bool andLogic = conditionalSkip.setLogic?.toLowerCase() == 'and';
    var results = conditionalSkip.set_
            ?.map((set) => skipSet(set, skipContext, evalDate))
            .toList() ??
        [];
    return andLogic ? results.every((res) => res) : results.any((res) => res);
  }

  bool skipSet(VaxSet set_, SkipContext skipContext, VaxDate evalDate) {
    bool andLogic = set_.conditionLogic?.toLowerCase() == 'and';
    var conditionResults = set_.condition
            ?.map((condition) => evaluateCondition(condition, evalDate, set_))
            .toList() ??
        [];
    return andLogic
        ? conditionResults.every((res) => res)
        : conditionResults.any((res) => res);
  }
  ...
  bool evaluateCountLogic(
    ...
    int requiredCount = int.tryParse(requiredCountStr ?? '0') ?? 0;
